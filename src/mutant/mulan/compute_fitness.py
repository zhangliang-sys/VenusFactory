...
def main():
    ...
    print('args.output_dataset_folder', args.output_dataset_folder)
    ...
    if os.path.exists(scoring_filename):
        ...
        pdb_filenames = mapping_protein_seq_DMS["DMS_id"][mapping_protein_seq_DMS["DMS_id"] == DMS_id].values[0] + ".pdb"
        if isinstance(pdb_filenames, str):
            pdb_filenames = [pdb_filenames]  # 转换为列表形式
        # print('len(pdb_filenames)', len(pdb_filenames))
        pdb_ranges = mapping_protein_seq_DMS["pdb_range"][mapping_protein_seq_DMS["DMS_id"] == DMS_id].values[0].split(
            '|')

        # print('mapping_protein_seq_DMS',mapping_protein_seq_DMS)
        print('pdb_filenames', pdb_filenames)
        print('pdb_ranges', pdb_ranges)
        
        model_scores = []
        for pdb_index, pdb_filename in enumerate(pdb_filenames):
            print(len(dataset.protein_names))
            # print(pdb_filename)
            dataset_id_index = [i for i, name in enumerate(dataset.protein_names) if name[0] == pdb_filename[:-4]]
            # print('dataset_id_index', dataset_id_index)
