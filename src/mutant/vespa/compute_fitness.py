...
if __name__ == "__main__":
    ...
    if not args.skip_VESPA_computation:
        ...
        #     filename = filename.replace(".a2m", ".a3m")
        #     # print(filename)
        ...
        #                     seq_wt_file.write(line+"\n")
        all_WT_sequences_fasta = args.vespa_tmp_dir + os.sep + args.wt_fasta_file
        wt_fasta_dir = "/home/zliang/project/proteinsquare/data/fasta/cleaned_wt"
        index_map = {"indices":[],"MSA_filename":[]}
        
        # 清空目标文件（如果存在）
        open(all_WT_sequences_fasta, 'w').close()
        
        for i, filename in enumerate(MSA_filenames):
            index_map["indices"].append(i)
            
            # 从MSA文件名获取DMS_id（去掉.a2m或.a3m后缀）
            dms_id = filename.replace(".a2m", "").replace(".a3m", "")
            index_map["MSA_filename"].append(filename)
            
            # 构建对应的FASTA文件路径
            fasta_path = os.path.join(wt_fasta_dir, f"{dms_id}.fasta")
            
            # 如果FASTA文件存在，则将其内容追加到目标文件
            if os.path.exists(fasta_path):
                with open(fasta_path, 'r') as src_file:
                    content = src_file.read().strip()
                    if content:  # 确保内容不为空
                        with open(all_WT_sequences_fasta, 'a') as dest_file:
                            dest_file.write(content + "\n")
            else:
                print(f"Warning: FASTA file not found for {dms_id}")
        # print(["vespa",args.wt_fasta_file,"--prott5_weights_cache", args.cache_location, "--vespa"])
    ...
    # VESPA_scores_folder = "/home/zliang/project/proteinsquare/output/VESPA"
    for DMS_id in list_DMS:
        ...
        output_dir = "/home/zliang/project/proteinsquare/output/small/VESPA"
        final_scores_VESPA.to_csv(output_dir+os.sep+DMS_filename,index=False)
        print(final_scores_VESPA)
        
        # 清理临时文件
        try:
            if os.path.exists(args.vespa_tmp_dir):
                shutil.rmtree(args.vespa_tmp_dir)
                print(f"Successfully cleaned up temporary directory: {args.vespa_tmp_dir}")
