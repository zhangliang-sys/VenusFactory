...
def main(args):
    ...
    with torch.no_grad():
        ...
        for ix, protein in enumerate(dataset_valid):
            # print('protein', len(protein['seq_chain_H']))
            score_list = []
            global_score_list = []
            all_probs_list = []
            all_log_probs_list = []
            ...
            # print("Debug chain_M information:")
            # print(f"chain_M shape: {chain_M.shape}")
            # print(f"chain_M sum per position: {chain_M[0].sum(dim=0)}")  # 每个位置的掩码和
            # print(f"Number of 1s in chain_M: {chain_M[0].sum()}")  # 应该等于目标链的长度
            # print(f"chain_id_dict: {chain_id_dict}")  # 查看链的设置
            ...
            for fc in range(loop_c):
                ...
                # print(f"S_input shape: {S_input.shape}")
                # print(f"input_seq_length: {input_seq_length}")
                # print(f"chain_M shape: {chain_M.shape}")
                # print(f"Number of 1s in chain_M: {chain_M.sum().item()}")  # 1的总数
                ...
                # print('Debug seq str information:')
            ...
            # sanity check
            if not all([global_seq_list[i] == fasta_seqs[i] for i in range(len(global_seq_list))]):
                print("Sequence mismatch detected:")
                for i in range(len(global_seq_list)):
                    if global_seq_list[i] != fasta_seqs[i]:
                        print(f"Index {i}:")
                        print(f"PDB sequence:  {global_seq_list[i]}")
                        print(f"FASTA sequence:{fasta_seqs[i]}")
                        print(f"Length: PDB={len(global_seq_list[i])}, FASTA={len(fasta_seqs[i])}")
                raise AssertionError("Sequence mismatch between PDB and FASTA")
