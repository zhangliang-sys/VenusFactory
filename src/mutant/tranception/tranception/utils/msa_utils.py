...
def process_msa_data(MSA_data_file):
    ...
    return msa_data
def convert_lists_to_strings(sequence_dict):
    """
    将字典中的字符列表转换为字符串
    
    Args:
        sequence_dict: 字典，值为字符列表
        
    Returns:
        转换后的字典，值为字符串
    """
    return {key: ''.join(value) for key, value in sequence_dict.items()}
def get_one_hot_sequences_dict(msa_data,MSA_start,MSA_end,vocab):
    ...
    # print(num_sequences_msa,MSA_end-MSA_start,vocab_size)
...
def get_msa_prior(MSA_data_file, MSA_weight_file_name, MSA_start, MSA_end, len_target_seq, vocab, retrieval_aggregation_mode="aggregate_substitution", filter_MSA=True, verbose=False):
    ...
    if filter_MSA:
        ...
        for sequence_name in list_sequence_names:
            ...
            # print(seq_hot.shape,ref_sequence_hot.shape)
            # print()
    ...
    if MSA_weight_file_name is not None:
        ...
        # print('MSA_EVE.seq_name_to_sequence:',len(MSA_EVE.seq_name_to_sequence['>UniRef100_UPI0007752225/0-67']),MSA_EVE.seq_name_to_sequence['>UniRef100_UPI0007752225/0-67'])
        # print('msa_data:',msa_data['>UniRef100_UPI0007752225/0-67'])
        #We scan through all sequences to see if we have a weight for them as per EVE pre-processing. We drop them otherwise.
        dropped_sequences=0
        # list_sequence_names = list(msa_data.keys())
        list_sequence_names = list(MSA_EVE.seq_name_to_sequence.keys())
        MSA_weight=[]
        for sequence_name in list_sequence_names:
    ...
    if retrieval_aggregation_mode=="aggregate_substitution" or retrieval_aggregation_mode=="aggregate_indel":
        msa_data = convert_lists_to_strings(MSA_EVE.seq_name_to_sequence)
