...
if __name__ == "__main__":
        ...
        with open(full_temp_folder + os.sep + MSA_upper_file, "w") as f:
            ...
            for line in lines:
                ...
                if line[0] == ">":
                    # 如果已经有序列，先写入之前的序列
                    if current_seq:
                        # 处理序列：去除所有点号和空格，转换为单行
                        cleaned_seq = current_seq
                        f.write(cleaned_seq.upper() + "\n")
                        current_seq = ""
                    # 写入header
                    if not "/" in line:
                        newline = ">" + line[1:].strip().replace("_","").replace(".","") + "/" + str(MSA_start) + "-" + str(MSA_end) + "\n"
                        f.write(newline)
                    else:
                        f.write(line + "\n")
                else:
    ...
    os.makedirs(full_temp_folder + os.sep +"101",exist_ok=True)
    command = f"singularity exec --bind {args.GEMME_path} $gemme python {args.GEMME_path}/gemme.py {MSA_upper_file} -r input -f {MSA_upper_file} -m {mutant_temp_file} -N {args.nseqs}"
    print(command)
