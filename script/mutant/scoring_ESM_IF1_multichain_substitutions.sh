...
#PBS -l select=1:ncpus=6:ngpus=1:mem=48gb
...
# 为复合物设置专门的输出目录
export DMS_output_score_folder=${DMS_output_score_folder_subs}/ESM-IF1-multichain/
...
do
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "${protein_name},${DMS_index}" >> esm_if1_complex_failed_indices.txt
    fi
